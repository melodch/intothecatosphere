...
class Board:
    ...
    def create_reference_lines(self):
        for j in range(1, len(self.map)):
            for i in range(len(self.map)):
                if self.map[j][i] == 1:
                    #position = [i,j]                  
                    self.References.append(Reference(pg.image.load('reference.png'),(j* 10, (i - 1) * 10 - 4)))
        ...
        for y in range(0, height, 10):
            ...
            while num_on_this_lvl < rand_num:
                for x in range(5, width, h_spacing):
                    ...
                    elif self.map[x][y] == self.map[x][y + 10] == rand_ladder == 1 and \
    ...
    def redraw_screen(self, display_screen, score_label, lives_label, width, height):
        ...
        if self.game_state == 1:
            ...
            self.reference_group.draw(display_screen)
